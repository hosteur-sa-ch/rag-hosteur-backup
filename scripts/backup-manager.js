function BackupManager(config) {
    ...
    var Response = com.hivext.api.Response,
        ...
        SHELL_CODES = {},
        INSTALL_RS_SCRIPT = "install-restic.sh",
        AUTO_UPDATE_SCRIPT = "auto-update-ssl-cert.sh",
        SETTINGS_PATH = "opt/raghosteur/settings",
        DECREASE_UPDATE_DAYS = 10,
        REMOVE_UPDATE_DAYS = 90,
        SUPPORT_EMAIL = "noc@hosteur.com",
    ...
    nodeManager.setLogPath("var/log/ragbackup.log");
    nodeManager.setBackupPath("var/lib/jelastic/ragbackup");
    nodeManager.setCustomSettingsPath("var/lib/jelastic/ragbackup/settings-custom");
...
// A modifier
    me.reinstall = function reinstall(){
        ...
        me.logAction("StartPatchRSAutoUpdate");
        nodeManager.setBackupCSScript();
        resp = me.exec(me.backupScripts);

        if (resp.result != 0) {
            me.logAction("ErrorPatchRSAutoUpdate", resp);
            return resp;
        }
        ...
        me.logAction("EndPatchRSAutoUpdate", resp);
        ...
        return me.execAll([
            ...
            [ me.cmd, 'rm -rf %(paths)', {
                paths : [
                    ...
                    nodeManager.getScriptPath(INSTALL_RS_SCRIPT),
...
// A modifier
    ...
    me.createScript = function createScript() {
        var url = me.getScriptUrl("install-backup.js"),
            scriptName = config.scriptName,
            scriptBody,
            resp;
        ...
        try {
            ...
            if (resp.result == Response.OK) {
                ...
                me.logAction("StartConfigureRSUpdate");
    ...
    me.evalScript = function evalScript(action) {
        ...
        if (me.getAddOnAction() == CONFIGURE) {
            me.logAction("EndConfigureRSUpdate", resp);
        }
